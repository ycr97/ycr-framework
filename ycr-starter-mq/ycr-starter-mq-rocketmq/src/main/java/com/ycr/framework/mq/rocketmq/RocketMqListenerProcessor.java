...
public class RocketMqListenerProcessor implements BeanPostProcessor, DisposableBean {
    ...
    private final ObjectProvider<RocketMqProperties> properties;
    private final ObjectProvider<RocketMqClientFactory> clientFactory;
    private final Environment environment;
    private final List<PushConsumer> consumers = new ArrayList<>();

    public RocketMqListenerProcessor(ObjectProvider<RocketMqProperties> properties,
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        ...
        PushConsumer consumer = clientFactory.getObject()
                .createPushConsumer(handler, group, topic, tag, annotation.consumeThreadCount());
    ...
    private String resolveGroup(String group) {
        String resolved = StringUtils.hasText(group) ? group : properties.getObject().getGroup();
    ...
    private String resolve(String topic, boolean enableSuffix) {
        ...
        RocketMqProperties currentProperties = properties.getObject();
        if (enableSuffix && StringUtils.hasText(currentProperties.getEnv())) {
            String suffixed = RocketMqConstants.SUFFIX + currentProperties.getEnv();
